...
const Dashboard = () => {
  ...
  return (
    <div className='pt-16'>
      ...
      {results[5].isLoading ? <Loader /> : results[5].isError ? <div>Something went wrong...</div> : <div className='pt-16 px-5 flex justify-center lg:gap-10 md:gap-10 gap-5'>
      ...
      <div className='max-w-[60rem] grid lg:grid-cols-[minmax(100px,_1fr),minmax(100px,_1fr)] md:grid-cols-[minmax(100px,_1fr),minmax(100px,_1fr)] grid-cols-[minmax(100px,_1fr)] gap-10  m-auto my-16'>
        {
    ...
    <Link className='' to={data[idx].link}>
      <div className="py-7 px-5 flex justify-between text-center border text-white-500  border-[#0f0e0e] bg-[#0f0e0e] hover:border-gray-800  hover:bg-[#141313]  font-medium delay-[30ms] hover:scale-[101%] ease-in transition-all rounded-2xl text-lg">
        <div className='flex gap-4  hover:white'>
          {response.isLoading ? <Loader /> : response.isError ? <div className='text-5xl'>NA</div> : <div className='text-5xl'>
